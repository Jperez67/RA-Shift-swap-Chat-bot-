const CALENDAR_CONFIG = {
  CALENDAR_ID: 'c_5e26b9f1f6b07436d199ee5fefa527cfacd406e68a7db73a30ddeb7aab275e16@group.calendar.google.com',
  MANAGE_GUESTS: true
};
...
syncCalendarGuestsForRow_(event, rowData);
...
function getEmailForRA_(raName) {
  const roster = getRosterLookup_();
  const key = normalizeName_(raName);

  if (!roster[key]) {
    return '';
  }

  return roster[key].email || '';
}

function syncCalendarGuestsForRow_(event, rowData) {
  if (!CALENDAR_CONFIG.MANAGE_GUESTS) return;

  const currentEmail = getEmailForRA_(rowData.currentRA);

  if (!currentEmail) {
    appendAuditLog_({
      action: 'Calendar Guest Sync Failed',
      shiftDate: rowData.dateKey,
      shiftSlot: rowData.shiftSlot,
      oldRA: '',
      newRA: rowData.currentRA,
      requestedBy: 'System',
      result: 'Error',
      notes: `No email found in Roster for ${rowData.currentRA}`
    });
  ...
  const roster = getRosterLookup_();
  const rosterEmails = Object.values(roster)
  ...
  guests.forEach(guest => {
    const guestEmail = String(guest.getEmail()).trim().toLowerCase();

    if (
      rosterEmails.includes(guestEmail) &&
      guestEmail !== currentEmailLower
    ) {
      event.removeGuest(guestEmail);
    }
  });

  const updatedGuests = event.getGuestList().map(guest =>
    String(guest.getEmail()).trim().toLowerCase()
  );

  if (!updatedGuests.includes(currentEmailLower)) {
    event.addGuest(currentEmail);
  }
}
